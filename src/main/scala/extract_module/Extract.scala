...
/**
  ...
  * Description:
  */
object Extract extends tLines with GFFutils {
  ...
  def main(args: Array[String]) {
    val parser = new scopt.OptionParser[Config]("extract-orfs") {
      ...
      opt[File]('o', "output-databse") required() action { (x, c) =>
        c.copy(outputDir = x)
  ...
  def extract(config: Config): Unit ={
    ...
    def constructDataBase(genome_id: String, fasta: File, gff: File, dir: File, global_orf_id: Int): Int = {
      ...
      val pw_sequences = new PrintWriter(dir + "/" + genome_id + ".orfs.sequences.fasta")
      //open fasta assembly
      ...
      /**
        ...
        * @return Unit
        */
      def outputToDB: (List[GFFLine], Int, atk.Record) => Unit = (overlaps, id, fasta_entry) => {
        //get maximal ORF information
        val (generic_name, start, end) = getMaximalORF(overlaps)
      ...
      def parseORFs(orf_id: Int,
        ...
        if (!assembly.hasNext) (orf_id, Y, Z)
          ...
          //get id
          val entry_id = current_entry.getDescription.substring(1)
          ...
          val (updated_Z, last_orf_ID, last_overlaps) = corresponding_orfs.foldLeft((Z, orf_id, List[GFFLine]())){
            case((local_Z,local_orf_id,overlaps), current_orf) => {
              ...
              //no overlapping annotations
                ...
                outputToDB(overlaps, local_orf_id, current_entry)
          ...
          //one last set of overlapping orfs at end of sequence
            ...
            //output to DB
            outputToDB(last_overlaps, last_orf_ID, current_entry)
            //get current sequence of ORFs
    ...
    println("Constructing databse: " + timeStamp)
    ...
    //close output files
    List(pw_Y, pw_Y_prime, pw_Z, pw_Z_prime, pw_id_mapping).foreach(_.close())
    println("Successfully completed!" + timeStamp)
  }



  def getMaximalORF: List[GFFLine] => (String,Int,Int) = gffs => {
